...
const AddProofWork = ({ isOpen, onRequestClose }) => {
  const [projectTitle, setProjectTitle] = useState("");
  const [projectDescription, setProjectDescription] = useState("");
  const [skillsUsed, setSkillsUsed] = useState([]); // Correct type for skillsUsed
  ...
  const handleSkillKeyDown = (event) => {
    ...
    switch (event.key) {
      ...
      case "Tab":
        ...
        setFormData(prev => ({
          ...prev,
          skills_used: [...selectedSkills, newSkill].map(skill => skill.value).join(',')
        }));
        setSkillInputValue("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Failed to add proof of work");
      }
      toast.success("Proof of work added successfully!");
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to add proof of work. Please try again."
      );
      setFormErrors({
        submit: "Failed to add proof of work. Please try again.",
      });
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-96">
        ...
        {success && (
          <p className="text-green-500">Proof of work added successfully!</p>
        )}
