    ...
    >
      ...
      <nav className="flex-grow mt-8">
        <ul>
          ...
          {/* Bounties */}
          <li
            // onClick={() => handleActiveItem("team settings")}
            onClick={() => {
          ...
          >
            ...
            <CalendarIcon
              className={`h-6 w-6 transition-colors duration-300 ease-in-out ${
                activeItem === "messages" ? "text-green-500" : "text-gray-500"
              } ${isCollapsed ? "mx-auto" : "mr-4"}`}
            />
            {!isCollapsed && (
              <span
      ...
      {/* <nav className="mb-8 mx-2">
        <button
          ...
          className={`text-gray-800 flex justify-center gap-2 items-center mx-auto text-basebackdrop-blur-sm lg:font-semibold isolation-auto ${
            isCollapsed
              ? ""
              : "border-[#75EDA6] shadow-lg  bg-gray-50 border-2  rounded-full group"
        ...
        >
          {!isCollapsed && <span>Switch to User</span>}
          <svg
            className={`w-7 h-7 ${
              isCollapsed ? "mx-auto" : ""
