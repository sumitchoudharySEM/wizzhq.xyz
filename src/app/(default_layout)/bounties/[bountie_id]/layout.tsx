...
async function getBountyData(bountyId: string) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/bounty?slug=${bountyId}`, {
      method: "GET",
      headers: {
...
async function getPartnerData(partnerId: string) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/partners_listing/partner?id=${partnerId}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
...
// Generate metadata
export async function generateMetadata({ params }: { params: { bountie_id: string } }): Promise<Metadata> {
  const bountyData = await getBountyData(params.bountie_id);
  const partnerData = bountyData?.partner_id ? await getPartnerData(bountyData.partner_id) : null;
  ...
  if (bountyData?.reward) {
    ...
    if (rewards.prizes) {
      ...
      const remainingPrizes = rewards.prizes.slice(3).reduce((sum: number, prize: number) => sum + prize, 0);
      
      // Calculate bonus prize pool
      const bonusTotal = rewards.bonusPrize && 
  ...
  if (bountyData?.title) queryParams.set('title', encodeURIComponent(bountyData.title));
  if (partnerData?.profile_photo_url) queryParams.set('plogo', encodeURIComponent(partnerData.profile_photo_url));
  if (partnerData?.name) queryParams.set('pname', encodeURIComponent(partnerData.name));
  if (bountyData?.type) queryParams.set('type', encodeURIComponent(bountyData.type));
  queryParams.set('location', encodeURIComponent(bountyData?.location || 'Global'));
  ...
  const encodedUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/og/bounty?${queryParams.toString()}`;
